...
- [x] 기능 목록에 입력사항을 잘못 입력한 경우

### 매칭하기

- [x] 데이터 파일 읽어서 크루 멤버들 저장하기
- [x] 매칭할 미션 목록 보여주기
- [x] 매칭할 미션 입력 받기
  - [x] 매칭할 미션에 이미 매칭이 되어 있는 경우 재 매치여부 물어보기
    - [x] 재매치O : 초기화한 후에 다시 매칭받기
    - [x] 재매치X : 매칭할 미션 입력받기
- [x] 멤버들 무작위로 섞기
- [x] 섞은 멤버의 페어를 짝 지어주기
  - [x] 만약 같은 레벨의 다른 미션에서 같은 페어가 있다면 재시작
  - [x] 재시작이 4번째이면 에러 메세지 보내주기
- [x] 매칭 결과 출력하기

`예외`

- [x] 파일에 동명이인이 있는 경우
- [x] 섞기 라이브러리가 제대로 되지 않은 경우
- [x] 매칭할 미션 입력이 잘못되거나 목록에 없는 경우
- [x] 재시작이 4번째인 경우
...
- [x] 매칭 목록 입력 받기
- [x] 재시작이 4번째이면 에러 메세지
  - [x] 목록이 없는 경우 기능 입력 받기
  - [x] 목록이 있는 경우 해당 목록 보여주기
