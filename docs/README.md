...
- [x] 과정 종류에 따라 미션에 참여할 인원을 추출한다.

1. 무작위 매칭 만들기

- [x] missionUtils 패키지 메소드를 통해 무작위로 섞는다
- [x] 차례대로 2명씩 배열로 묶어서 넣는다.
  - [x] 만약 마지막에 세 명이 남는다면 같이 넣는다.

1. 매칭 중복 확인하기

- [x] 같은 레벨의 미션에 접근하여 매칭 정보가 있는 지 확인한다.
- [x] 매칭 정보가 있다면 같은 매칭이 있는지 확인한다.
- [x] 이미 존재하는 매칭을 모두 가져와서 매칭 중복을 검사한다.
  - [x] 같은 매칭이 있다면 다시 무작위로 섞는일 부터 한다 (1번으로)
    - [x] 만약 무작위 매칭을 3번 반복했으면 에러 메세지를 출력한다.
  - [x] 같은 매칭이 없다면 매칭 이력에 정보를 기입한다.

3. 매칭 결과 출력하기

### 매칭 조회

- [x] 과정에 맞는 storage를 찾는다.
  - [x] 없을 경우 error 메세지를 출력한다.
  - [x] 있을 경우 과거에 매칭된 결과를 출력한다.

### 매칭 초기화

- [x] 과정, 미션 정보를 바탕으로 storage를 초기화한다.
- [x] 초기화 메세지를 출력한다.
